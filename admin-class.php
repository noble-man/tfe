...
class itg_admin {
    ...
    public function get_nicename() {
        ...
        global $db;
        $info = $db->get_row("SELECT `nicename` FROM `utilisateurs` WHERE `username` = '" . $db->escape($username) . "'");
        if(is_object($info))
    ...
    public function get_email() {
        $username = $_SESSION['admin_login'];
        global $db;
        $info = $db->get_row("SELECT `email` FROM `utilisateurs` WHERE `username` = '" . $db->escape($username) . "'");
        if(is_object($info))
            return $info->email;
        else
            return '';
    ...
    private function _check_db($username, $password) {
        global $db;
        $user_row = $db->get_row("SELECT * FROM `utilisateurs` WHERE `username`='" . $db->escape($username) . "'");
